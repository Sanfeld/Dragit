<?php
function oceanquest_fishmisc($op2){
	global $session;
	$op = httpget('op');
	$op3 = httpget('op3');
	...
	if ($op2=='gauge'){
		if ($op=="fishingexpedition"){
			addnav("Continue","runmodule.php?module=oceanquest&op=fishingexpedition&loc=".$temp);
			if ($allprefs['fishbook']==1) addnav("Read Your Fishing Book","runmodule.php?module=oceanquest&op=docks&op2=readfishbook&op3=expedition&op4=$op3");
		}else{
			output("`nThe readings at location `^%s`7 are:`n",$op3);
			if ($allprefs['fishbook']==1){
				addnav("Research");
				addnav("Read Your Fishing Book","runmodule.php?module=oceanquest&op=docks&op2=readfishbook&op3=expeditiona&op4=$op3");
			}
		}
		output("`n`#Temperature: `7%s Degrees`n",$allprefs['temp'.$op3]);
		output("`%Depth: `7%s Feet`n",$allprefs['depth'.$op3]);
		output("`@Wind: `7%s Knots`n",$allprefs['wind'.$op3]);
	}
	if ($op2=="gold"){
		if ($op3=="1"){
	...
	if ($op2=="bodysearch"){
		if ($op3==1){
			if (is_module_active("alignment")){
				output("Searching a body is an `\$evil`7 act and your alignment suffers because of this.`n`n");
				increment_module_pref("alignment",-1,"alignment");
			}
			output("After a thorough search of the body, you figure that he was a soldier for the King of Pilinoria.");
			output("Not really being a doctor, you can't make a definitive diagnosis of murder, but the multiple stab wounds to the heart lead you to think it's possible he died due to foul play.");
			output("`n`nSomething's rotten in the state of Pilinoria!");
			output("`n`nThe only thing that you think may be useful from the body is an `%Imperial Badge`7 which you slip into your pocket.");
			output("`n`nThe captain notices your 'catch' and has the crew take it to the hold so it can be returned to the King of Pilinoria.");
			$allprefs['pass']=1;
	...
	if ($op2=="fish"){
		$tem=$allprefs['temp'.$op3];
		$dep=$allprefs['depth'.$op3];
		$win=$allprefs['wind'.$op3];
		...
		//switch(30){
			...
			case 18: case 15: case 13:
				if (($allprefs['copperring']==""||$allprefs['copperring']==0) && ($allprefs['opencave']=="" || $allprefs['opencave']==0)){
					$allprefs['copperring']=1;
					output("You feel a small tug on your line and haul up what appears to be a green ring about the size of an orange.");
					output("You discreetly wipe it off and notice that it's made of `2`bC`Qo`2p`Qp`2e`Qr`b`7.  You slip it into your pocket and pretend you didn't catch anything.");
				}else{
					output("You reel it in and pull in a very decent fish; good job!");
					$weight=e_rand(201,300);
				}
			...
			case 22:
				...
				addnav("`\$Keep Gold","runmodule.php?module=oceanquest&op=$op&op2=gold&op3=1");
				addnav("`^Ignore Gold","runmodule.php?module=oceanquest&op=$op&op2=gold&op3=2");
				addnav("`@Return Gold","runmodule.php?module=oceanquest&op=$op&op2=gold&op3=3");
				blocknav("runmodule.php?module=oceanquest&op=$op&op2=fish&op3=$op3");
				blocknav("runmodule.php?module=oceanquest&op=$op&op2=gauge&op3=$op3");
				blocknav("runmodule.php?module=oceanquest&op=$op&loc=".$temp);
			break;
			case 23:
				$fight=e_rand(1,3);
				if ($fight==1 && $session['user']['turns']>0){
					...
					addnav("Fight Crewman","runmodule.php?module=oceanquest&op=fishcrew");
					addnav("Walk Away","runmodule.php?module=oceanquest&op=$op&op2=walkaway&op3=$op3");
					blocknav("runmodule.php?module=oceanquest&op=$op&op2=fish&op3=$op3");
					blocknav("runmodule.php?module=oceanquest&op=$op&op2=gauge&op3=$op3");
					blocknav("runmodule.php?module=oceanquest&op=$op&loc=".$temp);
				}else{
			...
			case 24:
				...
				addnav("Reel in the Shark","runmodule.php?module=oceanquest&op=fishshark");				
			break;
			case 25:
			...
			case 26:
				...
				if ($rand==1 && $allprefs['fishbook'] == 1){
					output("`n`nAs you gather the fish though, it kicks your pocket and you lose your fishing book!!");
					$allprefs['fishbook']=0;
					debuglog("lost their fishing book but gained a turn while fishing in the Ocean Quest.");
				}else debuglog("gained a turn while fishing in the Ocean Quest.");
				$weight=e_rand(201,300);
				$session['user']['turns']++;
			break;
			case 27:
				...
				if (is_module_active("lumberyard")){
					...
					if ($allprefsl['squares']>=3){
						...
						$number++;
						addnav("Pay Wood","runmodule.php?module=oceanquest&op=$op&op2=damagepay&op3=1");
					}
				}
				if ($session['user']['gems']>=1){
					...
					$number++;
					addnav("Pay a Gem","runmodule.php?module=oceanquest&op=$op&op2=damagepay&op3=2");
				}
				if ($session['user']['gold']>=300){
					$type=3;
					output("You can give me `^300 gold`& to help pay for repair costs.");
					$number++;
					addnav("Pay Gold","runmodule.php?module=oceanquest&op=$op&op2=damagepay&op3=3");
				}
				if ($session['user']['turns']>=2){
					$type=4;
					output("I think you could spend `@2 turns`& fixing it and that would be helpful.");
					$number++;
					addnav("Spend Turns","runmodule.php?module=oceanquest&op=$op&op2=damagepay&op3=4");
				}
				...
				if ($number==1){
					...
					blocknav("runmodule.php?module=oceanquest&op=$op&op2=damagepay&op3=$type");
			...
			case 28:
				...
				if ($pass==1 && ($allprefs['pass']==""||$allprefs['pass']==0)) $op3=1;
				else $op3=2;
				addnav("Search the Body","runmodule.php?module=oceanquest&op=$op&op2=bodysearch&op3=$op3");
				addnav("Leave the Body","runmodule.php?module=oceanquest&op=$op&op2=bodysearch&op3=3");
			break;
			case 29:
