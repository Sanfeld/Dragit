 ...
 *
 * Modifed by Hex:
 *	- Made it so you only sober up a bit on newday
 */

function drinks_getmoduleinfo(){
	$info = array(
		"name"=>"Exotic Drinks",
		"author"=>"John J. Collins<br>Heavily modified by JT Traub<br>Contributions by `!Hex",
		"category"=>"Inn",
		"download"=>"http://dragonprime.net/users/Hex/drinks.zip",
		"settings"=>array(
...
function drinks_dohook($hookname,$args) {
	...
	case "newday":
		...
		$drunktmp=get_module_pref("drunkeness");
		$drunktmp-=e_rand(1,17);
		if($drunktmp<=0){
			$drunktmp=-1;
		}
		set_module_pref('drunkeness',$drunktmp);
		break;
...
function drinks_editor(){
	...
	superusernav();
	addnav("Drink Editor");
	addnav("Add a drink","runmodule.php?module=drinks&act=editor&op=add");
	$op = httpget('op');
	$drinkid = httpget('drinkid');
	$header = "";
	if ($op != "") {
		addnav("Drink Editor Main","runmodule.php?module=drinks&act=editor");
		if ($op == 'add') {
			$header = translate_inline("Adding a new drink");
		} else if ($op == 'edit') {
			$header = translate_inline("Editng a drink");
		}
	} else {
	...
	if($op=="del"){
		...
		module_delete_objprefs('mounts', $drinkid);
		db_query($sql);
		$op = "";
		httpset('op', "");
	}
	...
	if ($op==""){
		...
		for ($i=0;$i<db_num_rows($result);$i++){
			...
			rawoutput("<td nowrap>[ <a href='runmodule.php?module=drinks&act=editor&op=edit&drinkid=$id'>$edit</a>");
			addnav("","runmodule.php?module=drinks&act=editor&op=edit&drinkid=$id");
			if ($row['active']) {
				rawoutput(" | <a href='runmodule.php?module=drinks&act=editor&op=deactivate&drinkid=$id'>$deac</a>");
				addnav("","runmodule.php?module=drinks&act=editor&op=deactivate&drinkid=$id");
			} else {
				rawoutput(" | <a href='runmodule.php?module=drinks&act=editor&op=activate&drinkid=$id'>$act</a>");
				addnav("","runmodule.php?module=drinks&act=editor&op=activate&drinkid=$id");
			}

			rawoutput(" | <a href='runmodule.php?module=drinks&act=editor&op=del&drinkid=$id' onClick='return confirm(\"$conf\");'>$del</a> ]</td>");
			addnav("","runmodule.php?module=drinks&act=editor&op=del&drinkid=$id");
	...
	$subop= httpget("subop");
	if($op=="edit"){
		addnav("Drink properites", "runmodule.php?module=drinks&act=editor&op=edit&drinkid=$drinkid");
		...
		if ($subop=="module") {
			...
			rawoutput("<form action='runmodule.php?module=drinks&act=editor&op=save&subop=module&editmodule=$module&drinkid=$drinkid' method='POST'>");
			module_objpref_edit('drinks', $module, $drinkid);
			$mostrecentmodule = $oldmodule;
			rawoutput("</form>");
			addnav("", "runmodule.php?module=drinks&act=editor&op=save&subop=module&editmodule=$module&drinkid=$drinkid");
		} elseif ($subop=="") {
	...
	if (($op == "edit" || $op == "add") && $subop=="") {
		rawoutput("<form action='runmodule.php?module=drinks&act=editor&op=save' method='POST'>");
		addnav("","runmodule.php?module=drinks&act=editor&op=save");
