...
function dagminotaur_dohook($hookname,$args){
	...
	case "dagquests":
		if (get_module_pref("status")==5) {
			...
			if ($goldgain && $gemgain) {
				output("`3You hand Dag the minotaurs head, and Dag pays you the bounty of `^%s gold`3 and a pouch of `%%s %s`3!",$goldgain,$gemgain,translate_inline(($gemgain==1)?"gem":"gems"));
			} elseif ($gemgain) {
				output("`3You hand Dag the minotaurs head, and Dag pays you the bounty of a pouch of `%%s %s`3!",$gemgain,translate_inline(($gemgain==1)?"gem":"gems"));
			} elseif ($goldgain) {
				output("`3You hand Dag the minotaurs head, and Dag pays you the bounty of `^%s gold`3!",$goldgain);
			} else {
				output("`3You hand Dag the minotaurs head, but Dag cannot find the bounty to pay you!");
			}
...
function dagminotaur_run(){
	...
	case "minotaurfighting":
		...
		if ($victory) {
			...
			output("`2You lop off the beasts head, and stash the gruesome thing in your backpack.");
			// Reward flag
